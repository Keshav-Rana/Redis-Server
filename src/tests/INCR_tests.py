...
class INCR_tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_normalCase(self):
        ...
        response_displayed_to_user = '(integer) 2'
        self.assertEqual(self.resp_service.deserialiser(splitted_data[2], response), response_displayed_to_user)

    def test_edgeCase(self):
        # increment a value where the key does not exist, first set it to 0 then increment
        input_cmd = "INCR test2"
        serialised_input = "*2\r\n$4\r\nINCR\r\n$5\r\ntest2\r\n"
        self.assertEqual(self.resp_service.serialiser(input_cmd), serialised_input)
    ...
    def test_invalidCase2(self):
        ...
        input_cmd = "INCR test2 test3"
        serialised_input = "*3\r\n$4\r\nINCR\r\n$5\r\ntest2\r\n$5\r\ntest3\r\n"
        self.assertEqual(self.resp_service.serialiser(input_cmd), serialised_input)

        # try to run INCR command without any arguments
        input_cmd_two = "INCR"
        ...
        response_from_redis = "-ERR wrong number of arguments for 'incr' command\r\n"
        # split serialised data
        splitted_data = self.resp_service.serialiser(input_cmd).split('\r\n')
        cmd_service = CommandService(splitted_data, self.redis_db)
        response = cmd_service.makeResponse()
        ...
        # error - second case
        response_from_redis_two = "-ERR wrong number of arguments for 'incr' command\r\n"
        # split serialised data
        ...
        response_displayed_to_user = "(error) ERR wrong number of arguments for 'incr' command"
        self.assertEqual(self.resp_service.deserialiser(splitted_data[2], response), response_displayed_to_user)

        response_displayed_to_user_two = "(error) ERR wrong number of arguments for 'incr' command"
