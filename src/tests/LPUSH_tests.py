...
class LPUSH_tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_normalCase(self):
        input_cmd = 'LPUSH test one two three'
        serialised_input = "*5\r\n$5\r\nLPUSH\r\n$4\r\ntest\r\n$3\r\none\r\n$3\r\ntwo\r\n$5\r\nthree\r\n"
        ...
        self.assertEqual(self.redis_db.get("test")[0], "three")
        self.assertEqual(self.redis_db.get("test")[1], "two")
        self.assertEqual(self.redis_db.get("test")[2], "one")

    # no arguments with command
    def test_noArguments(self):
        input_cmd = 'LPUSH test'
        serialised_input = "*2\r\n$5\r\nLPUSH\r\n$4\r\ntest\r\n"
        self.assertEqual(self.resp_service.serialiser(input_cmd), serialised_input)

        response_from_redis = "-ERR wrong number of arguments for 'lpush' command\r\n"
        # split serialised data
        splitted_data = self.resp_service.serialiser(input_cmd).split('\r\n')
        cmd_service = CommandService(splitted_data, self.redis_db)
        response = cmd_service.makeResponse()
        ...
        response_displayed_to_user = "(error) ERR wrong number of arguments for 'lpush' command"
        self.assertEqual(self.resp_service.deserialiser(splitted_data[2], response), response_displayed_to_user)

    # adding values to wrong key
    def test_wrongKey(self):
        input_cmd = 'LPUSH mockKey one'
        serialised_input = "*3\r\n$5\r\nLPUSH\r\n$7\r\nmockKey\r\n$3\r\none\r\n"
        self.assertEqual(self.resp_service.serialiser(input_cmd), serialised_input)
